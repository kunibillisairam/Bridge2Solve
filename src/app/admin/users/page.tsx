    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-brandgray-border/60 pb-5">
        <h1 className="text-xl font-bold text-primary uppercase tracking-wide flex items-center gap-2.5">
          <Users className="h-6 w-6 text-primary shrink-0" /> Registered Users & Organizations Registry
        </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          return (
            <Card key={i} className={`border p-4 bg-white shadow-subtle ${card.color.split(" ").slice(2).join(" ")}`}>
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold text-brandgray-text block uppercase tracking-wider">{card.label}</span>
                <Icon className={`h-4 w-4 shrink-0 ${card.color.split(" ")[0]}`} />
              </div>
              <span className="text-2xl font-extrabold text-primary block mt-1.5 leading-none">{card.value}</span>
            </Card>
          );
        })}
      </div>
      ...
      <Card className="border border-brandgray-border bg-white shadow-subtle">
        <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="relative w-full md:w-80">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-450 shrink-0" />
            <input
              type="text"
              placeholder="Search by name, organization, state, email..."
              className="text-xs bg-slate-50 border border-brandgray-border pl-9 pr-3 py-2 rounded-lg w-full focus:outline-none focus:ring-1 focus:ring-primary font-medium placeholder-slate-400"
              value={searchQuery}
            ...
            {searchQuery && (
              <button 
                ...
                className="absolute right-2.5 top-2.5 text-slate-450 hover:text-primary"
              >
                <X className="h-4 w-4" />
              </button>
            )}
          ...
          {/* Role Filters */}
          <div className="flex flex-wrap gap-1 border-b border-slate-100 text-xs w-full md:w-auto">
            {["All", "Citizens", "Universities", "Industry / CSR", "Admins"].map((role) => (
              <button
                key={role}
                onClick={() => setSelectedRoleFilter(role)}
                className={`py-1.5 px-3.5 font-bold border-b-2 transition-all rounded-t text-xs ${
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-brandgray-border text-[10px] font-bold text-brandgray-muted uppercase tracking-wider">
                  <th className="py-3 px-4 font-bold">Name / Organization</th>
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="py-3 px-4">
                        <span className="font-bold text-primary block leading-snug">{displayOrg}</span>
                        <span className="text-[10px] text-brandgray-muted font-semibold block mt-0.5">{displayName}</span>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4 text-brandgray-text font-semibold select-all">{u.email}</td>
                      <td className="py-3 px-4 text-brandgray-text">
                        <span className="flex items-center gap-1">
                          <MapPin className="h-3 w-3 text-slate-400 shrink-0" />
                          <span className="truncate max-w-[150px]">{u.location || "Not reported"}</span>
                        </span>
                      </td>
                      <td className="py-3 px-4 text-brandgray-muted font-medium">
      ...
      {selectedUser && selectedUserStats && (
        <div className="fixed inset-0 bg-black/55 z-55 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg border border-brandgray-border max-w-xl w-full shadow-2xl flex flex-col max-h-[85vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="px-5 py-4 border-b border-brandgray-border bg-slate-50 flex justify-between items-center">
              <div>
            ...
            {/* Modal Content */}
            <div className="p-5 space-y-5 text-xs text-brandgray-text">
              {/* Profile Card Header */}
            ...
            {/* Modal Footer */}
            <div className="px-5 py-3 border-t border-brandgray-border bg-slate-50 text-right">
